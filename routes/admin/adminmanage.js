...
exports.user = async function user(req) {
    try{
        ...
        const userinfo = await user_info.findOne({ where:  {ID} , attributes: ["UserNumber","ID","Belong","StudentID","Name","Nick","Email","LastIp","Comment","Score","created_at","solved_at"] });
        const usersolves = await solver_table.findAll({ where: {ID}, attributes: ["ChID","ChCategory"] });
        if (!usersolves) {
            return {
                userinfo: userinfo,
                solves: 0
