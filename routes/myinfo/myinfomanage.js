...
exports.myinfomanage = async function myinfomanage(req) {
    try {
        ID=req.user.ID
        const myinfo = await user_info.findOne({ where:  {ID} , attributes: ["Nick", "Comment","Name","StudentID","Email","belong","Score"] });
        const solves = await solver_table.findAll({ where: {ID}, attributes: ["ChID","ChCategory"] });
        if (!solves) {
            return {
                myinfo: myinfo,
                solves: 0
